...
const AttemptQuiz = () => {
    ...
    useEffect(() => {
        const answeredQuestion = providedAnswers.find(ans => ans.questionNo === page);
        if (answeredQuestion) {
    ...
    const handleRadioChange = (e) => {
        ...
        const existingAnswerIndex = providedAnswers.findIndex(ans => ans.questionNo === page);
        if (existingAnswerIndex !== -1) {
            ...
            setProvidedAnswers([...providedAnswers, { questionNo: page, ans: selectedValue }]);
        }
    ...
    return (
        <div className="h-full">
            {!isAttempQuiz &&
                <form className="flex flex-col gap-2 justify-center items-center h-full mx-auto" onSubmit={handleSubmit}>
                    <Input
                        size="large"
                        type="text"
                        ...
                        sx={{ width: "50%" }}
                    />
                    <Button type="submit" variant="outlined" sx={{ width: "fit-content" }}>submit</Button>
                </form>}
            ...
            {isAttempQuiz &&
                <div>
                    <div className="flex items-center px-5 pt-5 justify-between">
                        ...
                        <span>
                            {!quizCompleted && <CountdownTimer minutes={1 / 10} onComplete={handleTimeout} />}
                        </span>
                    </div>

                    <div className="p-5 border-2 m-5">
                        <FormLabel id="demo-error-radios" sx={{ fontSize: "23px", color: "black", fontWeight: "bold" }}>
                        ...
                        <div className="flex justify-end my-4">
                            {page === 10 && <Button variant="contained" onClick={() => setQuizCompleted(true)}>Submit Quiz</Button>}
                        </div>
                    ...
                    <div className="text-center mb-10">
                        {quizCompleted &&
                            <div className="border-2 max-w-md mx-auto py-6">
                                <p className="pb-5 font-bold text-xl upper">Thanks for taking the quiz!</p>
                                <p className="flex justify-between max-w-xs mx-auto mb-3 border-b border-black/30">
                                    Result: <span>{marks}/10</span>
                                </p>
                                <p className="flex justify-between max-w-xs mx-auto mb-3 border-b border-black/30">
                                    Taken time: <span>
                                        {calculateTime(quizStartAndEndTime)}
                                    </span>
                                </p>
                                <p className="flex justify-between max-w-xs mx-auto mb-3 border-b border-black/30">
                                    Quiz start time: <span>{new Date(quizStartAndEndTime.startTime).toLocaleTimeString()}</span>
                                </p>
                                <p className="flex justify-between max-w-xs mx-auto mb-3 border-b border-black/30">
                                    Quiz end time: <span>{new Date(quizStartAndEndTime.endTime).toLocaleTimeString()}</span>
                                </p>
                                <p className="flex justify-between max-w-xs mx-auto border-b border-black/30">
                                    Date: <span>{new Date(quizStartAndEndTime.startTime).toLocaleDateString()}</span>
                                </p>
                            </div>
                        }
                    </div>
