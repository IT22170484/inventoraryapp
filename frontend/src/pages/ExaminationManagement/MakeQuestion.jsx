...
const Form = ({ setShowForm, setQuestions, questions, editQuestion, setEditQuestion }) => {
    ...
    const validateForm = () => {
        ...
        formData.answers.forEach((answer, i) => {
            if (answer.trim() === "") {
                newErrors.answers[i] = `Answer ${i + 1} is required!`
            }
        });
    ...
    const handleAnswerChange = (i, value) => {
        ...
        setErrors(({ ...errors, answers: errors.answers.map((ans, index) => (i === index ? "" : ans)) }))
    }
            ...
            <Box component="form" onSubmit={formSubmit} sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                ...
                {formData?.answers?.map((answer, i) => (
                    <TextField
                        ...
                        value={answer}
                        helperText={errors.answers && errors.answers[i]}
                        FormHelperTextProps={{ sx: { color: "red" } }}
                    />
                ))}
                <div>
                    ...
                    />
                    {errors.correctAnswer && <span className="text-red-600 text-[12px] pl-[14px]">{errors.correctAnswer}</span>}
                </div>
                                                        ...
                                                        >
                                                            ...
                                                            <AccordionDetails>
                                                                <List>
                                                                    {row.answers.map((answer, i) => (
                                                                        <ListItem key={i} sx={{ display: "flex", gap: 1 }}>
                                                                            {answer === row.correctAnswer ? <Check fontSize="small" color="action" /> : <Clear fontSize="small" color="action" />}
                                                                ...
                                                                <div className="w-full flex justify-end gap-2">
                                                                    <Button variant="contained" onClick={() => handleEditQuestion(row._id)}>Edit</Button>
                                                                    <Button variant="contained" color="error" onClick={() => handleDeleteQuestion(row._id)}>Delete</Button>
                                                                </div>
                                                            </AccordionDetails>
                                                        </Accordion>
                                                    </TableCell>
