...
const AddInventory = () => {
    ...
    const formValidate = () => {
        ...
        if (inventoryDetails.category.trim() === "") {
            errors.category = "Category is required!"
            formIsValid = false
        }

        if (!inventoryDetails.price) {
            errors.price = "Price is required!"
            formIsValid = false
        }

        if (!inventoryDetails.quantity) {
            errors.quantity = "Quantity is required!"
            formIsValid = false
        }

        if (!inventoryDetails.file) {
                ...
                <Box component="form" encType="multipart/form-data" noValidate onSubmit={handleSubmit} sx={{ mt: 3 }}>
                    <Grid container spacing={2}>
                        <Grid item xs={12}>
                            ...
                            />
                            {errors.file && <span className="text-red-600 text-[12px] pl-[14px]">{errors.file}</span>}
                        </Grid>
                        <Grid item xs={12}>
                            <TextField
                                name="name"
